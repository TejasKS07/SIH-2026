...
- **Windows / Linux**: NVIDIA GPU with CUDA (optional, falls back to CPU)
- **macOS**: Apple Silicon (M1/M2/M3/M4) with Metal Performance Shaders (MPS) or Intel CPU
...
### 3. Run Privacy-Preserving Vision Pipeline & Evaluation

```bash
# Run test evaluation harness across all 4 scenarios (Payment, Passport, Medical, Bank Form):
python eval_harness.py

# Run offline mock test without downloading model weights:
python eval_harness.py --mock

# Launch the FastAPI Backend Service for the Chrome Extension:
python server.py
